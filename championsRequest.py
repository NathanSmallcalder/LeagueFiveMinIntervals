...
def getChampDetails(champion):
    DDRAGON = requests.get("http://ddragon.leagueoflegends.com/cdn/15.20.1/data/en_US/champion.json")
    DDRAGON = DDRAGON.json()
    DDRAGON = DDRAGON['data'][champion]
    DDRAGON['imageLink'] = "https://ddragon.leagueoflegends.com/cdn/15.20.1/img/champion/" + str(DDRAGON['id']) + ".png"
    ...
    for item in DDRAGON:
        ...
        for mastery in masteryScore:
            if int(temp['key']) == int(mastery['championId']):
                ...
                mastery['link'] = "https://ddragon.leagueoflegends.com/cdn/15.20.1/img/champion/" + temp['id'] +".png"


#Gets ChampionImage URLS for ChampionTable
def getChampImagesSingle(ChampId): # 
    DDRAGON = requests.get("https://ddragon.leagueoflegends.com/cdn/15.20.1/data/en_US/champion.json")
    DDRAGON = DDRAGON.json()
    DDRAGON = DDRAGON['data']
    ...
    for item in DDRAGON:
        ...
        if int(temp['key']) == (int(ChampId)):
            ChampId = "https://ddragon.leagueoflegends.com/cdn/15.20.1/img/champion/" + temp['id'] +".png"
            return ChampId
...
def getRunesImages(runesList): 
    ...
    data = requests.get("http://ddragon.leagueoflegends.com/cdn/15.20.1/data/en_US/runesReforged.json")
    data = data.json()
    try:
        for runes in data:
