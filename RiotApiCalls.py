...
def getMatchData(region,id,SummonerInfo,RankedDetails):
    ...
    MatchIDs = requests.get("https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/"+ SummonerInfo['puuid'] +  "/ids?start=0&count=10&api_key=" + API)
    MatchIDs = MatchIDs.json()
    data = getMatches("jp", MatchIDs, SummonerInfo,RankedDetails,mastery)
    return data

### Gets 5 MatchIds
def getMatchData5Matches(region,id,SummonerInfo,RankedDetails):
    mastery = getMasteryStats(region, SummonerInfo['puuid'])
    MatchIDs = requests.get("https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/"+ SummonerInfo['puuid'] +  "/ids?start=0&count=5&api_key=" + API)
    MatchIDs = MatchIDs.json()
    data = getMatches("", MatchIDs, SummonerInfo,RankedDetails,mastery)
    return data

### Gets x MatchIds
def getMatchIds(region,puuid):
    MatchIDs = requests.get("https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/"+ puuid +  "/ids?start=0&count=10&api_key=" + API)
    MatchIDs = MatchIDs.json()
    return MatchIDs
...
def getMatches(region,Summoner,MatchIDs,SummonerInfo,RankedDetails,mastery):
    ...
    for matchID in MatchIDs:
        ...
        MatchData = requests.get("https://asia.api.riotgames.com/lol/match/v5/matches/"+ matchID +"?api_key=" + api_key)
...
def calculateAvgTeamStats(Team,Region):
    ...
    for item in Team:
        summName = item
        SummonerInfo = getSummonerDetails("na1",summName)
...
def getMatchTimeline(region,id,puuid,data):
    ...
    for matchID in MatchIDs:
            ...
            MatchData = requests.get("https://asia.api.riotgames.com/lol/match/v5/matches/" + matchID + "/timeline?api_key=" + api_key)
